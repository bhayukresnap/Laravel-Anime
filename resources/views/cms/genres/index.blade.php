  ...
  <div class="demo-nav-title">
    <div class="media">
      <div class="media-body">
        <div class="display-6">Genres</div>
    ...
    <div class="pull-xs-right" role="toolbar">
      <a href="{{route('cms.genres.create')}}" class="btn btn-success" type="button">Add Data</a>
    </div>
  </div>
</div>
...
<div class="container-fluid">
  <div class="panel-wrapper">
    <div class="panel">
        ...
        <table class="table">
          ...
          <tbody>
            @foreach($genres as $genre)
              <tr>
                <td>{{$genre->name}}</td>
                <td>{{$genre->meta->slug}}</td>
                <td>{{$genre->created_at}}</td>
                <td>{{$genre->updated_at}}</td>
                <td>
                  <a class="btn btn-primary" href="{{route('cms.genres.edit', $genre->id)}}">
                    <i class="icon ion-edit"></i>
                  </a>
                  <form method="POST" action="{{route('cms.genres.destroy', $genre->id)}}">
                    @csrf
                    {{method_field('DELETE')}}
      ...
      <div class="table-bar">
        {{ $genres->appends(request()->input())->onEachSide(1)->links('vendor.pagination.bootstrap-4') }}
      </div>
    </div>
  </div>
</div>
