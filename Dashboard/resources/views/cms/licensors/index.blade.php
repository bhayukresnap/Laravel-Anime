  ...
  <div class="demo-nav-title">
    <div class="media">
      <div class="media-body">
        <div class="display-6">licensors</div>
    ...
    <div class="pull-xs-right" role="toolbar">
      <a href="{{route('cms.licensors.create')}}" class="btn btn-success" type="button">Add Data</a>
    </div>
  </div>
</div>
...
<div class="container-fluid">
  <div class="panel-wrapper">
    <div class="panel">
        ...
        <table class="table">
          ...
          <tbody>
            @foreach($licensors as $licensor)
              <tr>
                <td>{{$licensor->name}}</td>
                <td>{{$licensor->meta->slug}}</td>
                <td>{{$licensor->created_at}}</td>
                <td>{{$licensor->updated_at}}</td>
                <td>
                  <a class="btn btn-primary" href="{{route('cms.licensors.edit', $licensor->id)}}">
                    <i class="icon ion-edit"></i>
                  </a>
                  <form method="POST" action="{{route('cms.licensors.destroy', $licensor->id)}}">
                    @csrf
                    {{method_field('DELETE')}}
      ...
      <div class="table-bar">
        {{ $licensors->appends(request()->input())->onEachSide(1)->links('vendor.pagination.bootstrap-4') }}
      </div>
    </div>
  </div>
</div>
