  ...
  <div class="demo-nav-title">
    <div class="media">
      <div class="media-body">
        <div class="display-6">episodes</div>
    ...
    <div class="pull-xs-right" role="toolbar">
      <a href="{{route('cms.episodes.create')}}" class="btn btn-success" type="button">Add Data</a>
    </div>
  </div>
</div>
...
<div class="container-fluid">
  <div class="panel-wrapper">
    <div class="panel">
        ...
        <table class="table">
          ...
          <tbody>
            @foreach($episodes as $episode)
              <tr>
                <td>{{$episode->anime->name}} - {{$episode->name}}</td>
                <td>{{$episode->created_at}}</td>
                <td>{{$episode->updated_at}}</td>
                <td>
                  <a class="btn btn-primary" href="{{route('cms.episodes.edit', $episode->id)}}">
                    <i class="icon ion-edit"></i>
                  </a>
                  <form method="POST" action="{{route('cms.episodes.destroy', $episode->id)}}">
                    @csrf
                    {{method_field('DELETE')}}
      ...
      <div class="table-bar">
        {{ $episodes->appends(request()->input())->onEachSide(1)->links('vendor.pagination.bootstrap-4') }}
      </div>
    </div>
  </div>
</div>
