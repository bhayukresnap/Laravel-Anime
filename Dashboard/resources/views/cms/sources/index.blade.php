  ...
  <div class="demo-nav-title">
    <div class="media">
      <div class="media-body">
        <div class="display-6">sources</div>
    ...
    <div class="pull-xs-right" role="toolbar">
      <a href="{{route('cms.sources.create')}}" class="btn btn-success" type="button">Add Data</a>
    </div>
  </div>
</div>
...
<div class="container-fluid">
  <div class="panel-wrapper">
    <div class="panel">
        ...
        <table class="table">
          ...
          <tbody>
            @foreach($sources as $source)
              <tr>
                <td>{{$source->name}}</td>
                <td>{{$source->meta->slug}}</td>
                <td>{{$source->created_at}}</td>
                <td>{{$source->updated_at}}</td>
                <td>
                  <a class="btn btn-primary" href="{{route('cms.sources.edit', $source->id)}}">
                    <i class="icon ion-edit"></i>
                  </a>
                  <form method="POST" action="{{route('cms.sources.destroy', $source->id)}}">
                    @csrf
                    {{method_field('DELETE')}}
      ...
      <div class="table-bar">
        {{ $sources->appends(request()->input())->onEachSide(1)->links('vendor.pagination.bootstrap-4') }}
      </div>
    </div>
  </div>
</div>
